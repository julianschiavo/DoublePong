...
public extension Scene {
    ...
    public func introGame() {
        if isIntro {
            if nextIntro == 1 {
                introTitle                      = createLabel(title: "Welcome", size: 60.0, color: UIColor.white, hidden: false, bold: true)
                introTitle.frame                = CGRect(x: ((self.view?.frame.width)! / 2) - 200, y: 30 + (view!.frame.height / 2) - 130, width: 400, height: 72)
                introDescription                = createLabel(title: "Welcome to DoublePong - Pong, with even more paddles.", size: 20.0, color: UIColor.white, hidden: false)
                ...
                introButton                     = createButton(title: "Continue", color: UIColor(red: 0.0, green: 0.478431, blue: 1.0, alpha: 1.0), image: nil, action: #selector(self.changeSlide), transparent: false, x: ((self.view?.frame.width)! / 2) - 140, y: (self.view?.frame.height)! / 2 + 80, width: 280, height: 45, radius: 18)
